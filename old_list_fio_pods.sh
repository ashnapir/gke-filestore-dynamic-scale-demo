...
printf "%-45s | %-55s | %-55s | %-10s\n" "Pod" "Node" "Filestore Mount Status" "FIO Status"
echo "---------------------------------------------+---------------------------------------------------------+---------------------------------------------------------+------------"
...
while IFS=$'\t' read -r pod_name node_name; do
  ...
  if [ -z "$mount_source" ]; then
    mount_display="(Not Mounted)"
  elif [ "$mount_source" == "$EXPECTED_MOUNT" ]; then
    mount_display="${mount_source} (OK)"
  else
    mount_display="${mount_source} (WRONG - E: ${EXPECTED_MOUNT})"
  fi
  ...
  printf "%-45s | %-55s | %-55s | %-10s\n" "$pod_name" "$node_name" "$mount_display" "$fio_status"
done <<< "$PODS_NODES"
